...
function Board(props) {
	...
	function makeTieBoard(s,i) {
		return <Square key={i} s={s} val={props.node.boards[props.symm][s]} color={'yellow'} click={squareClick} />
	}
	...
	function makeWinBoard(s,i) {
		...
		return props.node.res.some(w=>symms[props.symm].indexOf(s)===symms[0].indexOf(w)) ?
			<Square key={i} s={s} val={transform[s]} color={'green'} click={squareClick} /> :
			<Square key={i} s={s} val={transform[s]} color={'red'} click={squareClick} />;
	}
